        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-4 pt-3 ">
            ...
            <div class="col-md-4 col-sm-6 col-xs-12">
                <div class="card text-white shadow mb-4">
                    <img src="{{ url('') }}/img/foto/berita/{{ $B->foto_berita }}" class="card-img" alt="...">
                    <div class="card-img-overlay text-center">
                    <h5 class="card-title">{{ $B->judul_berita }}</h5>
                    <p class="card-text"><small>Penulis : <b>{{ $B->penulis_berita }}</b> <br> {{ $B->format_tgl }}</small></p>
                    <a href="{{ route('berita.detail', $B->id_detail) }}" class="btn btn-outline-light tbl">Selengkapnya</a>
                    </div>
                </div>
            </div>
            @endforeach
